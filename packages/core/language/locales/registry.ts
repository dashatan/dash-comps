...
export const localeLoaders: Record<
  Language,
  () => Promise<LocaleSchema>
> = {
  en: async () => FALLBACK_LOCALE,
  fa: async () => (await import("@/lib/language/locales/fa.json")).default as LocaleSchema,
  ar: async () => (await import("@/lib/language/locales/ar.json")).default as LocaleSchema,
};
...
export async function loadAllLocales(): Promise<Record<Language, LocaleSchema>> {
  const entries = await Promise.all(
    (Object.keys(localeLoaders) as Language[]).map(async (lang) => [
      lang,
      await localeLoaders[lang](),
    ] as const),
  );
