...
export function categorizeColors(colors: ColorVar[]) {
  const background = colors.filter(
    (c) => c.name === '--background' || c.name === '--foreground' || c.name === '--card' || c.name === '--card-foreground'
  )

  const primary = colors.filter(
    (c) => c.name === '--primary' || c.name === '--primary-foreground' || c.name === '--secondary' || c.name === '--accent'
  )

  const utility = colors.filter((c) => c.name === '--muted' || c.name === '--muted-foreground' || c.name === '--border')
  ...
  if (savedColors) {
    try {
      ...
      if (Array.isArray(existingColors)) {
        const otherThemeColors = existingColors.filter((setting) => setting.theme !== currentTheme)
