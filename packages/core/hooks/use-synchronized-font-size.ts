...
export function useSynchronizedFontSize<T extends HTMLElement>(
  ...
  const calculateOptimalFontSize = (containerWidth: number, text: string, valueRef: HTMLParagraphElement) => {
  ...
  const updateFontSizes = () => {
    ...
    items.forEach((item, index) => {
      ...
      if (container && valueRef) {
        ...
        const optimalFontSize = calculateOptimalFontSize(containerWidth, valueText, valueRef)
