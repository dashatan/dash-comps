...
export default function CollapsedPinnedMenus() {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className='relative flex w-full items-center justify-center'>
      <Popover open={open}>
        <PopoverAnchor>
          <div className='hover:bg-sidebar-accent flex size-10 cursor-pointer items-center justify-center rounded-lg transition-all'>
            <Pin size={20} className='text-sidebar-icon' />
          </div>
        </PopoverAnchor>
        <PopoverContent side='right' className='border-sidebar-border bg-sidebar w-80 border p-0 shadow-sm'>
