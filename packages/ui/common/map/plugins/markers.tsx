...
export function MapMarkers<
  T extends { lat: string | number; long: string | number },
>({
  data,
    ...
    (map) => {
      ...
      data.forEach((item) => {
        ...
        if (popupItemsRef.current) {
          marker.bindPopup(
            formatMapOverlayHtml(
              popupTitleRef.current?.(item),
              popupItemsRef.current(item),
            ),
            popupOptions,
          );
  ...
  useMapPlugin(
    (map) => {
      ...
      const focusAt = (target: L.LatLng) => {
        ...
        const center = getMapCenterWithPixelOffset(
          map,
          target,
          zoom,
          selectionPanOffsetPx,
        );
