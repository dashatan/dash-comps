...
const RangeSlider = React.memo(
  ({
      ...
      (percentage: number) => {
        ...
        if (stepValues.length > 0) {
          ...
          const nearestStep = stepValues.reduce((prev, curr) => {
            return Math.abs(curr.value - rawValue) <
              Math.abs(prev.value - rawValue)
              ? curr
              : prev;
          });
    ...
    const minPercentage = React.useMemo(
      ...
      [localValue[0], getPercentage],
    );
    const maxPercentage = React.useMemo(
      () => getPercentage(localValue[1]),
      [localValue[1], getPercentage],
    );
    ...
    const activeRangeGradient = React.useMemo(() => {
      ...
      const minColorIndex = Math.floor(
        activeStartRatio * (gradientColors.length - 1),
      );
      const maxColorIndex = Math.ceil(
        activeEndRatio * (gradientColors.length - 1),
      );

      const activeColors = gradientColors.slice(
        Math.max(0, minColorIndex),
        Math.min(gradientColors.length, maxColorIndex + 1),
      );
    ...
    const handlePointerUp = React.useCallback(() => {
      ...
      if (
        lastOnChangeValueRef.current[0] !== localValue[0] ||
        lastOnChangeValueRef.current[1] !== localValue[1]
      )
        onChange(localValue);
        ...
        const updated: [number, number] =
          thumb === "min"
            ? [
                Math.max(
                  min,
                  Math.min(localValue[1] - 1, localValue[0] + step),
                ),
                localValue[1],
              ]
            : [
                ...
                Math.min(
                  max,
                  Math.max(localValue[0] + 1, localValue[1] + step),
                ),
              ];
        setLocalValue(updated);
        onChange(updated);
    ...
    const handleTrackClick = React.useCallback(
      (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const rect = containerRef.current.getBoundingClientRect();
        const newValue = getValueFromPercentage(
          ((e.clientX - rect.left) / rect.width) * 100,
        );
        const isCloserToMin =
          Math.abs(newValue - localValue[0]) <
          Math.abs(newValue - localValue[1]);
    ...
    return (
      ...
      >
        ...
        <div
          ref={containerRef}
          className={cn(
            "relative h-2 w-full cursor-pointer rounded-full bg-muted transition-colors",
          ...
          {stepValues.length > 0 && stepValues.length <= 31 && (
            <div className="absolute inset-0 flex items-center overflow-hidden">
              {stepValues.map((step, index) => {
                ...
                const isActive =
                  step.value >= localValue[0] && step.value <= localValue[1];
                return (
                  <div
                    key={index}
                    className={cn(
                      "absolute h-2 w-0.5 -translate-x-1/2 rounded-full transition-colors",
                      isActive ? "bg-primary/60" : "bg-foreground/10",
                    )}
                    style={{
                      left: `${stepPercentage}%`,
          ...
          <div className="relative mt-3 h-6">
            {stepValues.map((step, index) => {
              ...
              const shouldShowLabel =
                stepValues.length <= 10 ||
                index === 0 ||
                index === stepValues.length - 1 ||
                index % Math.ceil(stepValues.length / 10) === 0;
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "absolute top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 cursor-grab rounded-full border-2 border-primary bg-background shadow-md transition-all hover:scale-110 active:scale-125 active:cursor-grabbing",
