...
export const radioContainerVariants = cva("relative flex select-none", {
  variants: {
    size: {
      sm: "gap-1 text-xs",
      md: "gap-2 text-sm",
...
export const radioCircleContainerVariants = cva(
  "relative -mt-0.5 h-full min-w-4",
  {
    variants: {
      size: {
        sm: "gap-1 p-1 text-xs",
...
export const radioCircleVariants = cva(
  "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 opacity-0",
  {
    variants: {
      size: {
        sm: "w-3",
