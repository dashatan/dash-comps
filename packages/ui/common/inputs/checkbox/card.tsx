...
export interface CardCheckboxProps extends Omit<BasicCheckboxProps, 'aria-label' | 'className' | 'title'> {
...
const CardCheckbox = forwardRef<HTMLDivElement, CardCheckboxProps>(
  ({ title, subTitle, titleTemplate, className, id, ...checkboxProps }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'flex w-full cursor-pointer items-start rounded-lg transition-all duration-100 select-none',
          {
            'bg-secondary': checkboxProps.checked,
      ...
      >
        ...
        {(title || subTitle) && (
          <div className={cn('flex flex-col py-4', className?.titleContainer)}>
            {title && (
              <label htmlFor={`${checkboxId}-input`} className={cn('text-base font-semibold', className?.title)}>
                {title}
              </label>
            )}
            {subTitle && <span className={cn('text-muted-foreground text-sm font-normal', className?.subTitle)}>{subTitle}</span>}
          </div>
        )}
      </div>
    )
