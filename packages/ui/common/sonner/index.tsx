...
export interface SonnerProps extends ToasterProps, VariantProps<typeof sonnerVariants> {
  asChild?: boolean
}
...
export const toast = {
    ...
    sonnerToast.success(message, {
      ...
      description:
        options?.descriptions || options?.description ? (
          <div className='text-toast-success-foreground text-sm'>
      ...
      icon: <TickCircle size={24} color='var(--color-toast-success-foreground)' />,
    }),
  error: (message: string, options?: ToastOptions) =>
    sonnerToast.error(message, {
      ...
      description:
        options?.descriptions || options?.description ? (
          <div className='text-toast-error-foreground text-sm'>
            {options?.descriptions?.map((description) => (
              <div key={description}>{description}</div>
            ))}
            {options?.description && <div>{options?.description}</div>}
      ...
      icon: <CloseCircle size={24} color='var(--color-toast-error-foreground)' />,
    }),
  warning: (message: string, options?: ToastOptions) =>
    sonnerToast.warning(message, {
      ...
      description:
        options?.descriptions || options?.description ? (
          <div className='text-toast-warning-foreground text-sm'>
            {options?.descriptions?.map((description) => (
              <div key={description}>{description}</div>
            ))}
            {options?.description && <div>{options?.description}</div>}
      ...
      icon: <AlertTriangle size={24} color='var(--color-toast-warning-foreground)' />,
    }),
  info: (message: string, options?: ToastOptions) =>
    sonnerToast.info(message, {
      ...
      description:
        options?.descriptions || options?.description ? (
          <div className='text-toast-info-foreground text-sm'>
            {options?.descriptions?.map((description) => (
              <div key={description}>{description}</div>
            ))}
            {options?.description && <div>{options?.description}</div>}
...
const Toaster = React.forwardRef<HTMLDivElement, SonnerProps>(({ className, variant, ...props }, ref) => {
