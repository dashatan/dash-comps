...
import { EChartsTooltipService, TooltipContentGenerator } from '../services/tooltip'
 ...
 */
export function useEChartsTooltip(chartInstance: ECharts | null, options: UseEChartsTooltipOptions): void {
  const {
    contentGenerator,
    enabled = true,
    eventType = 'mouseover',
  ...
  useEffect(() => {
    ...
    const actualEventType = triggerOnClick || triggerOnRightClick ? 'click' : eventType
    ...
    const mouseOverHandler = tooltipService.createMouseOverHandler(contentGenerator, chartInstance, tooltipOptions, isMousemoveMode, {
      showDelay,
      hideDelay,
      followMouse,
    })
    const mouseOutHandler = tooltipService.createMouseOutHandler({ hideDelay })
    const mouseMoveHandler = tooltipService.createMouseMoveHandler(chartInstance, tooltipOptions, { followMouse })
    const clickHandler = triggerOnClick
      ? tooltipService.createClickHandler(contentGenerator, chartInstance, tooltipOptions, {
          showDelay,
          hideDelay,
          followMouse,
        })
    ...
    const rightClickHandler = triggerOnRightClick
      ? tooltipService.createRightClickHandler(contentGenerator, chartInstance, tooltipOptions, {
          showDelay,
          hideDelay,
          followMouse,
        })
