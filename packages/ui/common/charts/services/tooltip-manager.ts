...
export class EChartsTooltipManager {
  ...
  private lastOptions: { offsetX?: number; offsetY?: number; position?: 'right' | 'left' | 'top' | 'bottom' | 'auto' } | null = null
    ...
    if (!this.activeTooltip || !this.activeTooltip.parentNode) {
      ...
      this.scrollListener = () => {
        if (this.activeTooltip && this.lastPosition) {
          this.positionTooltip(this.activeTooltip, this.lastPosition, this.lastOptions || undefined)
        }
      }
      this.resizeListener = () => {
        if (this.activeTooltip && this.lastPosition) {
  ...
  private calculateAvailableSpace(
    ...
    const spaceRight = viewport.width - position.x - defaultOffset - tooltipWidth
    const spaceLeft = position.x - defaultOffset - tooltipWidth
    const spaceTop = position.y - defaultOffset - tooltipHeight
    const spaceBottom = viewport.height - position.y - defaultOffset - tooltipHeight
  ...
  private detectBestPosition(
    ...
    const spaces = this.calculateAvailableSpace(position, tooltipWidth, tooltipHeight, viewport)
    ...
    const bestPosition = Object.entries(scores).reduce((a, b) => (scores[a[0] as keyof typeof scores] > scores[b[0] as keyof typeof scores] ? a : b))[0]
    ...
    if (scores[bestPosition as keyof typeof scores] < 50) {
      ...
      if (spaces.right > spaces.left && spaces.right > spaces.top && spaces.right > spaces.bottom) {
        return 'right'
      }
  ...
  private positionTooltip(
    ...
    if (tooltipPosition === 'auto') {
      tooltipPosition = this.detectBestPosition(position, tooltipWidth, tooltipHeight, viewport)
    }
