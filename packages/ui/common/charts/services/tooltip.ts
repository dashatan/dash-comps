...
export type TooltipContentGenerator = (params: any) => React.ReactElement | string | null
...
export class EChartsTooltipService {
   ...
   */
  private getMousePosition(chartInstance: any, params: any): { x: number; y: number } {
    ...
    if (chartInstance?.getZr) {
      ...
      if (handler && handler.lastX !== undefined && handler.lastY !== undefined) {
  ...
  public createMouseOverHandler(
    ...
    return (params: any) => {
      ...
      if (areaName && areaName === this.lastHoveredAreaName && this.tooltipManager.getActiveTooltip()) {
      ...
      if (isMousemoveMode && activeTooltip && !this.hasParamsChanged(this.lastParams, params)) {
      ...
      if (isMousemoveMode && activeTooltip && this.lastContent) {
        if (React.isValidElement(content) && React.isValidElement(this.lastContent)) {
          // Simple comparison - if same type and key props, don't re-render
          const contentKey = (content as any).key || JSON.stringify((content as any).props)
          const lastKey = (this.lastContent as any).key || JSON.stringify((this.lastContent as any).props)
          if (contentKey === lastKey && !this.hasParamsChanged(this.lastParams, params)) {
        ...
        } else if (typeof content === 'string' && typeof this.lastContent === 'string') {
          if (content === this.lastContent && !this.hasParamsChanged(this.lastParams, params)) {
   ...
   */
  public createMouseOutHandler(config?: { hideDelay?: number }): (params: any) => void {
