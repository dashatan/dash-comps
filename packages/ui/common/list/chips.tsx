    ...
    <div className={cn('flex flex-wrap gap-2', props.className)}>
      {props.list?.map((item, index) => {
        return (
          <div
            key={index}
            className={cn('flex h-8 min-h-8 cursor-default justify-center gap-2 rounded-md border px-4', 'items-center', {
              'border-primary bg-primary': item.isActive,
              'border-background bg-background opacity-60': !item.isActive,
            })}
          >
            {props.icon || (item.icon && <div className='text-xs'>{item.icon || props.icon}</div>)}
