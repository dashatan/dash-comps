...
export default function TabScroller({ items, visibleCount = 6, value, onSelect, scrollTo = 'value' }: TabScrollerProps) {
  ...
  const getInitialState = () => {
    switch (scrollTo) {
      case 'value':
        ...
        if (findIndex > -1) {
          const mapFindIndexToStartIndex = findIndex > items.length / 2 ? findIndex + 1 - items.length / 2 : findIndex - items.length / 2

          if (Math.abs(mapFindIndexToStartIndex) > visibles / 2) {
            return mapFindIndexToStartIndex > 0 ? mapFindIndexToStartIndex - visibles / 2 : mapFindIndexToStartIndex + visibles / 2
          }
        }
      case 'center':
  ...
  useEffect(() => {
    if (parentRef.current) {
      setItemWidth((parentRef.current.offsetWidth - (visibles - 1) * 8) / visibles)
    }
  ...
  return (
    <div className='flex w-full items-center overflow-hidden'>
      ...
      <div ref={parentRef} className='flex flex-1 justify-center overflow-hidden'>
        <div ref={ref} className='relative inline-flex gap-2 transition-all duration-600 ease-out' style={{ transform: `translateX(${startIndex * step}%)` }}>
          {items.map((item, index) => (
            ...
            >
              <ScrollTabItem isActive={value === item.value} onClick={() => onSelect(item.value)} label={item.name} />
            </div>
          ))}
        </div>
      </div>
