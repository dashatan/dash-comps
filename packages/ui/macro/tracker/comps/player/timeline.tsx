...
export default function TrackerTimeLine() {
  ...
  const tickWidth = useMemo(
    () => (containerWidth - 16) / times.length,
    [containerWidth, times.length],
  );

  const evenlySpacedTimes = useMemo(
    () => calculateEvenlySpacedItems(times, containerWidth, 70),
    [containerWidth],
  );
      ...
      <div className="flex flex-col px-2" style={{ width: container.width }}>
        {tracksWithEvents?.map((track, i) => {
              ...
              >
                <div
                  className={classNames(
                    "h-1",
                    eventClasses({ color, hasEvent, outsideIran }),
                  )}
                ...
                {hasEmphasize && (
                  <div
                    className={classNames(
                      "bg-primary-500 h-full opacity-10",
          ...
          return (
            <div
              key={i}
              className="relative mt-0.5 flex h-12 w-full border-b last:border-b-0"
            >
              {tt}
            </div>
          );
  ...
  return (
    <div className="relative flex w-full flex-1 flex-col overflow-hidden px-6">
      <div
        className={classNames(
          "pointer-events-auto absolute top-0 left-0 flex h-10 w-full border-y border-gray-300 bg-gray-100",
        )}
      ></div>
      <div ref={container.ref} className="relative flex w-full flex-1 flex-col">
          ...
          thumbProps={{
            children: (
              <div className="flex w-full flex-col items-center justify-start">
                ...
                <div
                  className="bg-primary-600 -mt-1 w-0.5"
                  style={{
                    height: `${(tracksWithEvents?.length || 0) * 60 + 24}px`,
                  }}
                ></div>
              </div>
            ),
        ...
        >
          <div
            className={classNames(
              "pointer-events-auto top-0 left-0 flex h-10 w-full justify-between border-y border-gray-300 bg-gray-100 font-main text-xs",
            )}
          >
            {evenlySpacedTimes?.map((x, i, a) => {
              return (
                <div
                  key={i}
                  className="flex h-10 flex-col items-center justify-end px-1.5"
                  style={{ width: tickWidth }}
                >
                  <span>
...
function eventClasses({
  ...
  return {
    ...
    "bg-red-500":
      hasEvent && (outsideIran || color === "red" || color === colorWords.red),
    "bg-blue-500":
      hasEvent &&
      !outsideIran &&
      (color === "blue" || color === colorWords.blue),
    "bg-green-500":
      hasEvent &&
      !outsideIran &&
      (color === "green" || color === colorWords.green),
    "bg-yellow-500":
      hasEvent &&
      !outsideIran &&
      (color === "yellow" || color === colorWords.yellow),
    "bg-white":
      hasEvent &&
      !outsideIran &&
      (color === "white" || color === colorWords.white),
    "bg-gray-300":
      hasEvent &&
      !outsideIran &&
      (color === "gray" || color === colorWords.silver || color === undefined),
    "bg-black":
      hasEvent &&
      !outsideIran &&
      (color === "black" || color === colorWords.black),
    "bg-orange-500":
      hasEvent &&
      !outsideIran &&
      (color === "orange" || color === colorWords.orange),
    "bg-lime-500":
      hasEvent &&
      !outsideIran &&
      (color === "lime" || color === colorWords.lime),
    "bg-pink-500":
      hasEvent &&
      !outsideIran &&
      (color === "pink" || color === colorWords.pink),
    "bg-teal-500":
      hasEvent &&
      !outsideIran &&
      (color === "teal" || color === colorWords.teal),
    "bg-indigo-500":
      hasEvent &&
      !outsideIran &&
      (color === "indigo" || color === colorWords.indigo),
  };
}
