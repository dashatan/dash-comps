...
const CarPlate = forwardRef<HTMLDivElement, CarPlateInputProps>((props, ref) => {
  ...
  const [colorCode, setColorCode] = useState<string | undefined>(initialColorCode);
  const [localValues, setLocalValues] = useState<CarPlateInputValue | undefined>(
  ...
  useEffect(() => {
    if (defaultValue?.p3) {
      const code = lettersColorCodes.find((x) => x.letter === defaultValue.p3)?.colorCode;
  ...
  const handleKeyDown = useCallback(
    (i: number, e: React.KeyboardEvent<HTMLInputElement | HTMLDivElement>) => {
      if (disabled || readonly) return;
  ...
  const inputConfig: {
    ...
    { Comp: Number, max: 3 },
    {
      Comp: Number,
      max: 2,
      suffix: <IranPlateIcon color={foregroundVariable} className={cn("h-9 w-9")} />,
    },
  ];
    ...
    <Popover open={open} onOpenChange={(open) => setOpen(open)}>
      <PopoverAnchor className="h-full w-full">
        <div
          ...
          className={cn(
            "dir-ltr border-input-border flex h-14 w-fit overflow-hidden rounded-lg border-2 transition-none duration-100",
        ...
        >
          <div
            id={`${plateId}-flag`}
            className={cn(
              "bg-plate-flag group relative flex h-14 max-w-10 min-w-10 flex-1 flex-col items-start justify-center gap-1 p-2 text-white",
          ...
          >
            ...
            <div
              className={cn(
                "dir-ltr text-2xs flex flex-col font-bold",
            ...
            {isHovered && !disabled && !readonly && (
              <div className="bg-background/30 absolute inset-0 flex flex-1 flex-col justify-center backdrop-blur-sm duration-200">
      ...
      <PopoverContent
        ...
        className="dir-rtl"
        style={{
          width: containerRef.current?.clientWidth || containerWidth || "100%",
          zIndex: 99999999,
        }}
      >
