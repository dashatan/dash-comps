...
/* ------------------------------ Main Function ----------------------------- */
export default function SimplePlate({
  onChange,
  id,
  ...props
}: SimplePlateInputProps) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "h-full rounded-lg border border-gray-800 dir-ltr",
      )}
    >
      {Array.from(new Array(8)).map((_, x) => {
        return (
          <div
            key={x}
            id={`${plateId}-label-${x + 1}`}
            className="flex h-full w-full border-s border-gray-200"
          >
            <SimpleText
              ref={(r) => {
                ref.current[x + 1] = r;
