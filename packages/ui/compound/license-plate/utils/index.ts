...
export function normalizePlateValueToRequest(plate: PlateValue) {
  ...
  for (const [key, val] of Object.entries(plate)) {
    if (key === "p3") {
      ...
      params[key as keyof PlateValueRequestParams] = val
        ? parseFloat(val)
        : undefined;
    }
  }
...
export function normalizePlateValueToCar(
  values: PlateValue = {},
): CarPlateInputValue {
...
export function normalizePlateValueToCarLocalValue(
  values: PlateValue = {},
): CarPlateInputValue {
  // Combine p1+p2, p4+p5+p6, p7+p8 as strings, handle undefined gracefully
  ...
  return {
    p1: [values.p1, values.p2].filter(Boolean).join("") || "",
    p2: values.p3
      ? plateLetters.letters.find(
          (x) => x.code === Number(values.p3) || x.letter === values.p3,
        )?.letter
      : "",
    p3: [values.p4, values.p5, values.p6].filter(Boolean).join("") || "",
    p4: [values.p7, values.p8].filter(Boolean).join("") || "",
...
export function normalizeCarToPlateValue(
  values: CarPlateInputValue = {},
): PlateValue {
