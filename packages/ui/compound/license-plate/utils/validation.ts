...
export const IRANIAN_PLATE_RULES: PlateValidationRule[] = [
  ...
  },
  {
    name: 'not_all_zeros',
    validate: (plate) => {
      const values = [plate.p1, plate.p2, plate.p4, plate.p5, plate.p6, plate.p7, plate.p8]
 ...
 */
export function validatePlate(plate: PlateValue, options: PlateValidationOptions = {}): PlateValidationResult {
  ...
  const missingFields = requiredFields.filter((field) => !plate[field as keyof PlateValue])
  ...
  if (normalizedPlate.p4 === '0' && normalizedPlate.p5 === '0' && normalizedPlate.p6 === '0') {
...
export function validatePlatesFromExcel(
...
): { validPlates: PlateValue[]; invalidPlates: PlateValue[]; errors: string[]; warnings: string[] } {
  ...
  for (let i = 0; i < data.length; i++) {
    if (validPlates.length >= maxPlates) {
      warnings.push(`Maximum plate limit (${maxPlates}) reached. ${data.length - i} remaining plates will be ignored.`)
      break
    }
  ...
  if (validPlates.length > 0 && invalidPlates.length > 0) {
    warnings.push(`Found ${validPlates.length} valid and ${invalidPlates.length} invalid plates`)
  }
...
function parsePlateFromRow(row: any[]): PlateValue | null {
  ...
  if (row.length === 8 && row.every((cell) => cell !== undefined && cell !== null)) {
    return {
      p1: row[0]?.toString(),
      p2: row[1]?.toString(),
      p3: row[2]?.toString(),
  ...
  if (row.length === 5) {
    ...
    if (typeof letter === 'string') {
      const letterCode = plateLetters.letters.find((l) => l.letter === letter)?.code
      if (letterCode) {
        return {
          p1: row[4]?.[0]?.toString(),
          p2: row[4]?.[1]?.toString(),
...
export function formatPlateForDisplay(plate: PlateValue): string {
  ...
  const letter = plateLetters.letters.find((l) => l.code === Number(p3))?.letter || p3
 ...
 */
export function arePlatesEqual(plate1: PlateValue, plate2: PlateValue): boolean {
  const fields: (keyof PlateValue)[] = ['p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'p7', 'p8']
