...
const Comp = forwardRef<HTMLInputElement, NumberProps>(({ onChange, onFinish, suffix, containerWidth, id, ...props }, ref) => {
  ...
  return (
    <div className={cn('flex w-full flex-1 flex-col items-center justify-center gap-1 overflow-hidden', props.className)}>
      {!!suffix && <div className={cn('flex h-3/12 max-h-3/12 w-full flex-1 items-center justify-center')}>{suffix}</div>}
      <input
        ref={ref}
        {...props}
        id={inputId}
