...
export class ClusterPointsService {
  ...
  private onDeviceClickCallback?: (deviceId: number, deviceName?: string, location?: { lat: number; lng: number }) => void
  ...
  public setOnDeviceClick(callback?: (deviceId: number, deviceName?: string, location?: { lat: number; lng: number }) => void): void {
   ...
   */
  private pointsToGeoJSON(points: ClusterPoint[], weightField?: string): GeoJSONFeatureCollection {
    return {
      type: 'FeatureCollection',
      features: points.map((point) => {
        const weight = weightField
          ? (point.properties?.[weightField] != null ? Number(point.properties[weightField]) : 1)
          : 1
        return {
          type: 'Feature',
          geometry: {
   ...
   */
  private createCustomClusters(points: ClusterPoint[], gridSize: number = 2): GeoJSONFeatureCollection {
    // Group points by grid cells
    const gridMap = new Map<string, { count: number; lng: number; lat: number; ids: (string | number)[] }>()
    ...
    points.forEach((point) => {
      ...
      const stateIdNum = typeof stateId === 'string' ? Number(stateId) : stateId
    ...
    console.log(`[Custom Clustering] Created ${gridMap.size} clusters from ${points.length} points, gridSize=${gridSize.toFixed(2)}`)
  ...
  public setData(
    ...
    console.log(`Setting data: zoom=${zoom}, useCustomClustering=${useCustomClustering}, points=${points.length}, weightField=${weightField ?? 'none'}`)
   ...
   */
  private setBuiltInClusteringData(map: mapLibreGl.Map, points: ClusterPoint[], weightField?: string): void {
    ...
    const clusterCountTextField = weightField ? ['to-string', ['get', 'weight_sum']] : '{point_count_abbreviated}'
    ...
    if (!map.getLayer(LAYER_CLUSTER_COUNT)) {
      ...
      map.setLayoutProperty(LAYER_CLUSTER_COUNT, 'text-field', clusterCountTextField as any)
    }
    ...
    if (!map.getLayer(LAYER_UNCLUSTERED)) {
          ...
          try {
            ...
            customMarker.onload = () => {
              try {
                ...
                console.error('[Camera Icon] Failed to add custom marker:', err)
              }
            }
          } catch (err) {
      ...
      if (existingLayer && existingLayer.type === 'circle' && map.hasImage(CAMERA_ICON_ID)) {
        createIconLayer()
      }
    }
  }
   ...
   */
  private setCustomClusteringData(map: mapLibreGl.Map, points: ClusterPoint[], zoom: number, weightField?: string): void {
    ...
    pointsByProvince.forEach((provincePoints, stateId) => {
      ...
      const count = weightField
        ? provincePoints.reduce(
            (sum, p) => sum + (p.properties?.[weightField] != null ? Number(p.properties[weightField]) || 1 : 1),
            0
          )
        : provincePoints.length
      const displayCount = Number.isFinite(count) ? Math.round(count) : provincePoints.length
    ...
    const activeSourceIds = new Set(Array.from(pointsByProvince.keys()).map((id) => `province-${id}`))
    ...
    console.log(`[Province Sources] Created ${pointsByProvince.size} province sources from ${points.length} points`)
  }
  ...
  public updateLayersVisibility(map: mapLibreGl.Map, zoom: number): void {
    ...
    try {
      ...
      if (map.getLayer(LAYER_CLUSTER)) {
        map.setLayoutProperty(LAYER_CLUSTER, 'visibility', showBuiltInClusters ? 'visible' : 'none')
      }
      if (map.getLayer(LAYER_CLUSTER_COUNT)) {
        map.setLayoutProperty(LAYER_CLUSTER_COUNT, 'visibility', showBuiltInClusters ? 'visible' : 'none')
      }
      if (map.getLayer(LAYER_UNCLUSTERED)) {
        map.setLayoutProperty(LAYER_UNCLUSTERED, 'visibility', showBuiltInClusters ? 'visible' : 'none')
      }
      ...
      this.provinceLayerIds.forEach((layerId) => {
        try {
          if (map.getLayer(layerId)) {
            map.setLayoutProperty(layerId, 'visibility', showBuiltInClusters ? 'none' : 'visible')
          }
        } catch (error) {
  ...
  public attachHandlers(map: mapLibreGl.Map): () => void {
    ...
    const handlePointClick = (e: any) => {
      ...
      if (this.onDeviceClickCallback && deviceId) {
        this.onDeviceClickCallback(deviceId, deviceName, lat && lng ? { lat, lng } : undefined)
      }
    ...
    const provinceClickHandlers: Array<{ layerId: string; handler: (e: any) => void }> = []
    const provinceHoverHandlers: Array<{ layerId: string; enter: () => void; leave: () => void }> = []
    ...
    this.provinceLayerIds.forEach((layerId) => {
      ...
      if (layerId.endsWith('-layer')) {
        ...
        provinceHoverHandlers.push({ layerId, enter: setPointer, leave: resetCursor })
      }
    })
