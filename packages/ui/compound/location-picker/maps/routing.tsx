...
export default function RoutingMap() {
  ...
  const [getRoutingRequest, { isLoading, isFetching }] =
    useLazyGetRoutingQuery();
  ...
  const fetchRoute = useCallback(
    (params: { lat1: number; lng1: number; lat2: number; lng2: number }) => {
      getRoutingRequest({ ...params, devices: enrichedDevices ?? [] }, true)
        ...
        .then((res) => {
          const alternatives: RouteAlternative[] = res.alternatives.map(
            (alt) => ({
          ...
          const { destinationName, originName } = getRouteName(
            firstDevices,
            params,
          );
          ...
          setRouting({
            routes: res.alternatives.map((alt, index) => {
              ...
              return {
                ...
                title: t("locationPicker.routeTitle", {
