...
export default function FileUploader({ className, ...props }: React.ComponentProps<typeof FileUploaderTyped> & { className?: string }) {
  const { t } = useLanguage()
  return (
    <FileUploaderTyped
      {...props}
      ...
      hoverTitle={' '}
      dropMessageStyle={{ background: 'color-mix(in srgb, var(--color-muted) 10%, transparent)', border: 'none' }}
    >
      <div
        className={cn(
