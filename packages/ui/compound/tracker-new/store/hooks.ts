...
import { TrackerStoreContext } from "@dash/ui/compound/tracker-new/store/context";
...
} from "@dash/ui/compound/tracker-new/store/selectors";

export function useTrackerStore<T>(selector: (state: TrackerStore) => T): T {
  const store = useContext(TrackerStoreContext);
  if (!store) {
    throw new Error("useTrackerStore must be used within TrackerNewProvider");
  }
...
export function usePanelEvents() {
  return useTrackerStore(useShallow((s) => selectPanelEvents(s)));
}

export function useActiveEmphasizes() {
  return useTrackerStore(useShallow((s) => selectActiveEmphasizes(s)));
}

export function useTracksWithEventsFiltered() {
  return useTrackerStore(useShallow((s) => selectTracksWithEventsFiltered(s)));
}
