...
import { ChangeTag, ColumnProps, TableData } from '@components/compound/table/types'
import { cn } from '@/lib'
...
export const TH = memo(({ col, onchange, loading, index, columnHover, hoveredColumnIndex, onColumnHover, style, className }: THProps) => {
  const table = useFormContext<TableData>()
  const sortField = table.watch('sortField')
  const sortOrder = table.watch('sortOrder')
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn('flex w-full items-center gap-2', { 'cursor-pointer': col.sortable }, className?.l2, col.headerClassName?.l2)}
