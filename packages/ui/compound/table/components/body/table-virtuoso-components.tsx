...
export type VirtualTableRowContext = {
  ...
  rowProps?: {
    ...
    className?:
      | string
      | ((data: Record<string, unknown>, api: TableStoreApi) => string);
...
export type VirtualTableContext = VirtualTableRowContext &
  VirtualTableComponentsContext;
...
export const VirtualizedTableRow = memo(
  ...
  >(({ item, context, children, className, ...props }, ref) => {
    ...
    const { rightClickMenu, loading, rowProps, dataKey, expandOnNewRow, api } =
      context;
    const {
      getRowClassName,
    ...
    const isSelected =
      itemDataKey !== undefined && selected?.includes(itemDataKey);
    const expanded =
      item.kind === "row" &&
      isRowExpanded(item.item, dataKey, expandedRows, loading);

    const dynamicClassName =
      getRowClassName?.(item.item, api) ??
      (typeof classNameProp === "function"
        ? classNameProp(item.item, api)
        : undefined);
...
function ColGroup({ ctx }: { ctx?: VirtualTableComponentsContext }) {
...
>(({ context, ...props }, ref) => (
  <tr ref={ref} {...props}>
    <td
      ...
      className="h-16 border-b border-table-border bg-table"
    />
  </tr>
));
...
export function createVirtuosoComponents(): TableComponents<
  TableVirtualRow,
  VirtualTableContext
> {
  return {
