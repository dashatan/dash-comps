import { ColumnProps, tableDefaultState, TableProps } from '@/components/compound/table/types'
import EmptyTemplate from './empty'
import TableRow from './table-row'
import TD from './TD'
import { cn, makeArray } from '@/lib'
import { ContextMenu, ContextMenuContent, ContextMenuTrigger } from '@/components/common/context-menu'
...
export default function Body({
  ...
  const mockRows = makeArray(rowsCount <= 20 ? rowsCount : 20).map((_, i) => ({ id: i + 1 }))
  ...
  return (
    <tbody>
      {noResult ? (
        ...
        data?.map((item, index) => {
          const itemDataKey =
            item && dataKey ? (item as Record<string, unknown>)[dataKey as string] as string | number : undefined
          const expanded =
              ...
              {rightClickMenu && !loading ? (
                <>
                  <ContextMenu>
                    ...
                    <ContextMenuContent className="border-0 p-0">{rightClickMenu(item)}</ContextMenuContent>
                  </ContextMenu>
                  {expanded && rowExpansionTemplate && expandOnNewRow && (
                    <tr className="bg-table">
                      <td className="border-table-border border-b" colSpan={columns?.length}>
                        {rowExpansionTemplate(item)}
                      </td>
                    </tr>
                  )}
