...
export function DividerHor(props: DivProps) {
  return (
    <div {...props} className={cn("h-full w-px bg-border", props.className)} />
  );
}
...
export function Section(props: DivProps & { title?: string }) {
  return (
    <div
      {...props}
      className={cn(
        "flex h-full w-full items-start gap-2 p-4",
        props.className,
      )}
    >
      <span className="text-xs font-semibold whitespace-nowrap text-hint">
        {props.title} :
      </span>
      {props.children}
    </div>
