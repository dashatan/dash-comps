...
import { isDragScrollTarget } from "./utils/is-drag-scroll-target";
...
function measureHeaderMinWidth(th: HTMLTableCellElement) {
  ...
  const chrome =
    th.getBoundingClientRect().width - label.getBoundingClientRect().width;
  return Math.max(MIN_COL_WIDTH, measureIntrinsicWidth(label, chrome));
}

function measureCellContentWidth(td: HTMLTableCellElement) {
  const inner =
    td.querySelector<HTMLElement>('[data-slot="body-cell-content"]') ??
...
function measureColumnMinWidth(
  ...
  return Math.max(
    ...
    measureBodyMinWidth(table, colIndex),
  );
}
...
export function TableComponent({
  ...
  const columnFieldsKey = useMemo(
    () =>
      initialColumns
        ?.map((c) => `${c.field ?? ""}:${c.defaultInactive ? 1 : 0}`)
        .join("|") ?? "",
    [initialColumns],
  );
  ...
  const defaultActiveFields = useMemo(
    () =>
      initialColumns?.flatMap((x) =>
        !x.defaultInactive && x.field ? (x.field as string) : [],
      ) ?? [],
    [columnFieldsKey, initialColumns],
  );

  const activeColumns = storeActiveColumns?.length
    ? storeActiveColumns
    : defaultActiveFields;
  ...
  useLayoutEffect(() => {
    ...
    const ths = scrollRef.current?.querySelectorAll<HTMLTableCellElement>(
  ...
  function setColumnWidth(index: number, width: number) {
    ...
    scrollRef.current?.querySelectorAll("table").forEach((table) => {
  ...
  function handleColumnResizeStart(e: React.PointerEvent, index: number) {
    ...
    const table = scrollRef.current?.querySelector("table");
    if (!th || !col || !table) return;
    e.preventDefault();
    const startX = e.clientX;
    const startWidth = th.getBoundingClientRect().width;
    const key = colKey(col, index);
    const minWidth =
      columnMinWidthsRef.current[key] ??
      measureColumnMinWidth(th, index, table);
    ...
    const onMove = (ev: PointerEvent) => {
      setColumnWidth(index, computeWidth(ev));
    };
  ...
  function resolveFrozenColumnsForTable(cols?: ColumnProps[]) {
    return resolveFrozenColumns(cols, isRtl, getNumericColWidth);
  }

  const resolvedColumns = useMemo(
    () => resolveFrozenColumnsForTable(columns),
  ...
  const tableContent = (
    ...
    >
      ...
      <div
        id="table-content"
        className={cn(
          "flex min-h-0 flex-1 flex-row gap-4 overflow-hidden",
      ...
      >
        <div
          ...
          id="table-inner"
          className={cn(
            "relative min-h-0 w-full min-w-0 flex-1 overflow-auto rounded-md border border-table-border",
