...
import {
  ...
  type TrackerInput,
  type TrackerOptions,
} from "@/components/compound/tracker";
...
function PresetDemo({
  ...
  return (
    <TrackerFrame label={label} description={description}>
      <Tracker input={input} options={options} className="h-full" />
    </TrackerFrame>
  );
}
...
export function TrackerPresets({ labels }: { labels: Record<string, string> }) {
  useEffect(() => {
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      ...
      <TrackerFrame label={labels.livePreview}>
        <Tracker
          key={JSON.stringify({ state, input: input.kind })}
          input={input}
          options={options}
          className="h-full"
        />
